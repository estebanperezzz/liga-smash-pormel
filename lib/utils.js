 ...
 * @returns {Date} - Lunes de esa semana a las 09:00
 */
 ...
 * @param {Date} date - Fecha de referencia
 * @returns {Date} - Viernes de esa semana a las 14:30
 */
 ...
 * (viernes 14:30 → lunes 09:00), donde no hay semana activa
...
export function isInWeekendGap(date) {
  ...
  if (day === 5 && t >= 14 * 60 + 30) return true; // Viernes desde 14:30
  if (day === 6) return true;                        // Sábado
  if (day === 0) return true;                        // Domingo
  if (day === 1 && t < 9 * 60) return true;          // Lunes antes de 09:00
  return false;
}
